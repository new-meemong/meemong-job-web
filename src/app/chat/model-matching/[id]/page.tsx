...
export default function ModelMatchingChatPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const [userChannel, setUserChannel] =
    useState<UserModelMatchingChatChannelType | null>(null);
  ...
  const { subscribeToMessages, sendMessage } = useModelMatchingChatMessageStore(
    (state) => ({
  ...
  useEffect(() => {
    const initializeChannel = async () => {
      ...
      const foundUserChannel = userModelMatchingChatChannels.find(
      ...
      if (userModelMatchingChatChannels.length > 0) {
        setError("사용자 채널 정보를 찾을 수 없습니다.");
      }
    };

    initializeChannel();
  }, [source, userModelMatchingChatChannels, params.id]);
  ...
  useEffect(() => {
    if (
      source === "app" &&
      userModelMatchingChatChannels.length === 1 &&
      !userChannel
    ) {
      setUserChannel(userModelMatchingChatChannels[0]);
    }
  }, [source, userModelMatchingChatChannels, userChannel]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await sendMessage({
        ...
        message: messageText,
        messageType: ModelMatchingChatMessageTypeEnum.TEXT,
      });
