...
const WorkType = () => {
  ...
  const options = [
    ...jobPostingOptions.workType,
    { key: "상관없음", value: "상관없음" }
  ];

  const selectedOption = getJobPostingFilterQuery("workType") || "상관없음";

  const handleSelect = (selectedOption: string | null) => {
    selectedOption = selectedOption === "상관없음" ? null : selectedOption;
    if (selectedOption) {
      addJobPostingFilterQuery(`workType=${selectedOption}`);
    } else {
      removeJobPostingFilterQuery("workType");
    }
  };
