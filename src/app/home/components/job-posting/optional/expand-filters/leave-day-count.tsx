...
const LeaveDayCount = () => {
  ...
  const options = [
    ...jobPostingOptions.leaveDayCount,
    { key: "상관없음", value: "상관없음" }
  ];

  const selectedOption =
    getJobPostingFilterQuery("leaveDayCount") || "상관없음";

  const handleSelect = (selectedOption: string | null) => {
    selectedOption = selectedOption === "상관없음" ? null : selectedOption;
    if (selectedOption) {
      addJobPostingFilterQuery(`leaveDayCount=${selectedOption}`);
    } else {
      removeJobPostingFilterQuery("leaveDayCount");
    }
  };
