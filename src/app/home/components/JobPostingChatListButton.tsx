...
const JobPostingChatListButton = () => {
  ...
  const handleClick = () => {
    router.push(`/chat/job-posting?userId=${UserID}&source=${source}`);
  };
  ...
  return (
    <Container>
      <WriteButton onClick={handleClick}>
        <WriteIcon />
        <WriteButtonText>{`구인구직채팅`}</WriteButtonText>
      </WriteButton>
    </Container>
  );
};

export default JobPostingChatListButton;
